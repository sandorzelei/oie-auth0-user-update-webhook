...
/***/ function(module, exports, __webpack_require__) {
	...
	function getLogsFromAuth0(domain, client_id, token, take, from, cb) {
		...
		Request.get(url).set('Authorization', 'Bearer ' + token).set('Accept', 'application/json').query({ q: "client_id=" + client_id }).query({ take: take }).query({ from: from }).query({ sort: 'date:1' }).query({ per_page: take }).end(function (err, res) {
			if (err || !res.ok) {
